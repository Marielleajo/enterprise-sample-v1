...
const EditRequestAPI = (props) => {
  ...
  const getClientInfo = async () => {
    try {
      let response = await GET_API_BY_ID({
        token: props?.token,
        clientId: props?.dataID,
      });
      let data = response?.data?.data?.api;
      setState((prevState) => ({
        ...prevState,
        name: data?.name,
        description: data?.description,
        requestType: data?.typeTag,
        enableNumberConvertion: data?.convertInternationalFormat,
        countryCode: data?.countryRecordGuid,
        trafficType: data?.deliveryTypeTag,
        enableIpProtection: data?.isProtectedIp,
        apiWhiteList:
          data?.apiWhiteIpList?.length > 0
            ? data?.apiWhiteIpList?.map((item) => {
                const { ip } = item;
                return { Ip: ip };
              })
            : [{ Ip: "" }],
  ...
  const createAPI = async () => {
    ...
    const { t, setToggleEditIntegration, dataID, setLoading, getAPIs } = props;
    setLoading(true);
    const postData = {
      ChannelGuid: channels[0]?.recordGuid,
      apiGuid: dataID,
      name,
      typeTag: requestType,
      ...
      DeliveryTypeTag: trafficType,
    };
    try {
      let response = await EDIT_SMS_API({
        token,
        postData,
      });
      showSnackbar(t(response?.data?.message || "Successfully Edited API"));
      setToggleEditIntegration(false);
      getAPIs({});
    } catch (e) {
                ...
                <List>
                  ...
                  {formik.values.enableNumberConvertion && (
                    <ListItem>
                      <ListItemText
                        primary="Country"
                        secondary={formik.values.countryCode}
                      />
                    </ListItem>
                  )}
                  {formik.values.enableIpProtection &&
              ...
              <Button
                onClick={() => {
                  if (activeTab > 0) {
                    ...
                    props.setToggleEditIntegration(false);
                  }
                }}
...
export default connect(
  mapStateToProps,
  null
)(withTranslation("translations")(EditRequestAPI));
