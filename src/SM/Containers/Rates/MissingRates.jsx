...
function MissingRates({ t }) {
  ...
  const [manageMissingRate, setmanageMissingRate] = useState(false);
  ...
  const getAllRates = async () => {
    setLoading(true);
    try {
      let response = await GET_ALL_MISSING_RATES({
        token,
        search: "",
      ...
      const data =
        response?.data?.data?.missingRatePlans?.length > 0
          ? response?.data?.data?.missingRatePlans?.map((item) => ({
              ...item,
            }))
          : [];
      SetData(data);
  ...
  const exportAllRates = async () => {
    setLoading(true);
    try {
      let response = await EXPORT_MISSING_RATES({
        token,
        search: "",
        ChannelGuid: channelGuid ? channelGuid : null,
        ServiceGuid: serviceGuid ? serviceGuid : null,
      ...
      link.href = url;
      link.setAttribute("download", `Rates.csv`); // Set the desired file name
    ...
    <>
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      renderCell: (params) => {
                        return (
                          <>
                            <Tooltip title="Add Missing Rate">
                              <IconButton
      ...
      {manageMissingRate && (
        <MuiModal
          ...
          open={manageMissingRate}
          width="500px"
          id="edit-contact-form"
          handleClose={() => setmanageMissingRate(false)}
        >
          <ManageMissingRates
            type="edit"
            loading={loading}
            setLoading={setLoading}
            setManageAddRate={setmanageMissingRate}
...
export default withTranslation("translations")(MissingRates);
