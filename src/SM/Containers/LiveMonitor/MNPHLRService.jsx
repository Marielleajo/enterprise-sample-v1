...
import {
  EXPORT_LEVEL_ONE_MNPHLR,
  EXPORT_LEVEL_TWO_MNPHLR,
  GET_LEVEL_ONE_MNPHLR,
  GET_LEVEL_ONE_WIDGETS_MNPHLR,
  GET_LEVEL_TWO_MNPHLR,
  GET_LEVEL_TWO_WIDGETS_MNPHLR,
...
// Convert Hr to GMT
const convertHourToGMT = (hour) => {
  // Get the current date
  const currentDate = new Date();
  // Get the current timezone offset in milliseconds
  const currentOffset = currentDate.getTimezoneOffset() * 60 * 1000;
  // Convert hour to milliseconds and add the current offset
  const hourInMilliseconds = hour * 3600 * 1000;
  // Calculate the GMT hour by subtracting the current offset
  const gmtHour = new Date(hourInMilliseconds - currentOffset).getUTCHours();
  return gmtHour;
};

const MNPHLRService = ({ type, availableServices, selectedService }) => {
  ...
  const exportLevelOneData = async () => {
    try {
      let resellersResponse = await EXPORT_LEVEL_ONE_MNPHLR({
        token,
        search: SearchQuery,
        serviceTag: selectedService
          ? selectedService?.tag?.toLowerCase()
          : null,
        StartDate: startDate
        ...
        ServiceGuid: selectedService ? selectedService : null,
        ChannelGuid: null,
  ...
  const getLevelOne = async () => {
    setLoading(true);
    try {
      let response = await GET_LEVEL_ONE_MNPHLR({
        token,
        search: SearchQuery,
  ...
  const getLevelOneWidgets = async () => {
    setLoading(true);
    try {
      const startHourGMT = parseInt(startHour);
      const endHourGMT = parseInt(endHour);

      const startHourInSeconds = startHourGMT * 3600;
      const endHourInSeconds = endHourGMT * 3600;
      ...
      let response = await GET_LEVEL_ONE_WIDGETS_MNPHLR({
        token,
        search: SearchQuery,
        StartDate: startDate
          ? Math.floor(formatForDatetimeLocal(adjustToLocale(startDate)) / 1000)
      ...
      setWidgetData([
        {
          ...
          totalRows: response?.data?.data?.totalMessages ?? 0,
  ...
  const exportLevelTwoData = async () => {
    try {
      let resellersResponse = await EXPORT_LEVEL_TWO_MNPHLR({
        token,
        search: SearchQuery,
        serviceTag: selectedService
          ? selectedService?.tag?.toLowerCase()
  ...
  const getLevelTwo = async ({ clientGuid, isOnClick }) => {
    ...
    try {
      ...
      if (groupByValue == "provider") {
        response = await GET_LEVEL_TWO_MNPHLR({
          token,
          search: SearchQuery,
          pageSize: paginationModel?.pageSize,
          pageNumber: parseInt(paginationModel?.page) + 1,
  ...
  const getLevelTwoWidgets = async ({ clientGuid, isOnClick }) => {
    ...
    try {
      let response = "";
      if (groupByValue == "provider") {
        response = await GET_LEVEL_TWO_WIDGETS_MNPHLR({
          token,
          search: SearchQuery,
          StartDate: startDate
            ? Math.floor(
  ...
  const titleMap =
    selectedService?.tag?.toLowerCase() == "mnp"
      ? {
                ...
                <FormControlLabel
                  ...
                  control={
                    <Select
                      ...
                      className="live-data-select mt-2"
                ...
                monitorBeingViewed
                  ? groupByValue == "provider"
                    ? [
                        ...
                        ...(selectedService?.tag?.toLowerCase() == "hlr"
                          ? [
                              {
                                field: "totalActive",
