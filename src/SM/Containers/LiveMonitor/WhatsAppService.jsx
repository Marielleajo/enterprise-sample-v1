...
import {
  EXPORT_LEVEL_ONE_WHATSAPP,
  EXPORT_LEVEL_TWO_WHATSAPP,
  GET_LEVEL_ONE_WHATSAPP,
  GET_LEVEL_ONE_WIDGETS_WHATSAPP,
  GET_LEVEL_TWO_WHATSAPP,
  GET_LEVEL_TWO_WIDGETS_WHATSAPP,
...
const WhatsAppService = ({ type, availableServices, selectedService }) => {
  ...
  const exportLevelOneData = async () => {
    try {
      let resellersResponse = await EXPORT_LEVEL_ONE_WHATSAPP({
        token,
        search: SearchQuery,
  ...
  const getLevelOne = async () => {
    setLoading(true);
    try {
      let response = await GET_LEVEL_ONE_WHATSAPP({
        token,
        search: SearchQuery,
  ...
  const getLevelOneWidgets = async () => {
    ...
    try {
      ...
      let response = await GET_LEVEL_ONE_WIDGETS_WHATSAPP({
        token,
        search: SearchQuery,
        StartDate: startDate
          ? Math.floor(formatForDatetimeLocal(adjustToLocale(startDate)) / 1000)
  ...
  const exportLevelTwoData = async () => {
    try {
      let resellersResponse = await EXPORT_LEVEL_TWO_WHATSAPP({
        token,
        search: SearchQuery,
        serviceTag: selectedService
          ? selectedService?.tag?.toLowerCase()
  ...
  const getLevelTwo = async ({ clientGuid, isOnClick }) => {
    ...
    try {
      ...
      if (groupByValue == "provider") {
        response = await GET_LEVEL_TWO_WHATSAPP({
          token,
          search: SearchQuery,
          pageSize: paginationModel?.pageSize,
          pageNumber: parseInt(paginationModel?.page) + 1,
  ...
  const getLevelTwoWidgets = async ({ clientGuid, isOnClick }) => {
    ...
    try {
      let response = "";
      if (groupByValue == "provider") {
        response = await GET_LEVEL_TWO_WIDGETS_WHATSAPP({
          token,
          search: SearchQuery,
          StartDate: startDate
            ? Math.floor(
